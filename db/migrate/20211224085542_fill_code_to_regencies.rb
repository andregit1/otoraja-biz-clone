class FillCodeToRegencies < ActiveRecord::Migration[5.2]
  def change
    regencies =
      [
    ...
    regencies.each do |regency|
      p = Province.find_by(code: regency.second)
      next if p.blank?

      regency_name = regency.third.split(" ").delete_if {|i| i == "KOTA" || i == "KAB." || i == "ADM."}.last(2).join(" ")

      r = Regency.where(province_id: p.id).where("`name` LIKE ? ", "%#{regency_name}%")
      r = r.where(capital_name: "") if regency.third.include? "KOTA"

      r.first&.update(code: regency.first)
    end
  end
end
