class Admin::CheckinsController < Admin::ApplicationAdminController
  def list
    get_checkins
  ...
  def export_transaction_list
    get_checkins
    @checkins = @checkins.order(@search_sort_selected)

    respond_to do |format|
      format.xlsx {
        send_data render_to_string, filename: "list_transaksi_#{Time.zone.now.strftime('%Y%m%d')}_#{Time.parse(params[:start_date]).strftime('%Y%m%d')}-#{Time.parse(params[:end_date]).strftime('%Y%m%d')}.xlsx"
      }
    end
  end
  ...
  def get_checkins
    begin
      ...
      @checkins = if (@start_date.present? && @end_date.present?)
                    policy_scope(Checkin).where(shop_id: session[:default_user_shop]).where("CONVERT_TZ(#{@base_date}, 'UTC', 'ASIA/JAKARTA') BETWEEN '%s' AND '%s'", @start_date, @end_date)
                  else
                    policy_scope(Checkin).where(shop_id: session[:default_user_shop])
                  end
