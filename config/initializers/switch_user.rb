...
SwitchUser.setup do |config|
  ...
  config.available_users = { :user => -> { User.where(role:['staff', 'manager', 'shop_manager']) } }
  ...
  config.controller_guard = ->(_current_user, _request) { Rails.env.production? || Rails.env.staging? || Rails.env.development? || Rails.env.design? }

  # view_guard is a block,
  # if it returns true, the switch user select box will be shown,
  # else the select box will not be shown
  # if you switch from admin to "user", the current_user param is "user"
  config.view_guard = ->(_current_user, _request) { false }

  # redirect_path is a block, it returns which page will be redirected
  # after switching a user.
  config.redirect_path = ->(_request, _params) { _params['path'] || '/' }
